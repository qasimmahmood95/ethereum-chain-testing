...
## M1: deposit watcher and confirmation depth
...
- `src/watcher.ts`: pure state machine — input: new block headers and
  ...
  heights, because M2 depends on it.
...
- Watcher reorg logic: detect ancestry break by parent-hash mismatch,
  ...
  reconciliation in M5 re-checks this).
...
check is disabled locally (that becomes the M6 defect branch).

## M3: nonce management and broadcast idempotency
...
- `src/broadcaster.ts` + `src/nonce.ts`: intent store keyed by
  ...
  allocator, sign, persist `{intent, nonce, rawTx, hash}` *before* first
...
## M4: dropped and stuck transactions
...
## M5: ERC-20 deposits and reconciliation
...
- Watcher extended to token deposits — same state machine, same reorg
  handling (S15 reuses the M2 harness).
